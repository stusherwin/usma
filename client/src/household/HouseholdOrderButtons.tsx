...
export const HouseholdOrderButtons = (props: HouseholdOrderButtonsProps) => {
  ...
  return (
    <div className="flex flex-wrap content-start items-start">
      ...
      {canAddItem &&
        <button className="flex-no-grow flex-no-shrink ml-auto mt-2" onClick={e => { e.preventDefault(); e.stopPropagation(); props.startAdd()}}><Icon type="add" className="w-4 h-4 mr-2 fill-current nudge-d-2" />Add items</button>
      }
    </div>
  )
}