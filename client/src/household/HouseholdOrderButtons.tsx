...
export const HouseholdOrderButtons = ({unusedProducts, currentHouseholdOrder, className, newOrder, reopenOrder, abandonOrder, completeOrder, startAdd}: HouseholdOrderButtonsProps) => {
  ...
  const canAddItem = currentHouseholdOrder.isOpen && !!unusedProducts.length
  const canReopenOrder = !!currentHouseholdOrder.items.length && !currentHouseholdOrder.isOpen
  const canAbandonOrder = !!currentHouseholdOrder.items.length && currentHouseholdOrder.isOpen
  const canCompleteOrder = !!currentHouseholdOrder.items.length && currentHouseholdOrder.isOpen
