...
begin
  ...
  if exists(with t as (select to_regproc('upgrade_to_version') as x) select x from t where x is not null) then
    drop function upgrade_to_version(to_version int);
  end if;
  ...
  as $func$
    ...
    begin
      ...
      if curr_version is null then
        insert into db_upgrade (version) values (to_version);
      else
